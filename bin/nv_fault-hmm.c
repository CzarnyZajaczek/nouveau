...
static void
test(void *code, int threads, u32 stride, struct test_parm *parm, bool rw, u32 *outp, bool killed)
{
	u32 value = rand();
...
main(int argc, char **argv)
{
	...
	test(code, 1, 0x1000, parm, false, outp, false);
	free(outp);

	//XXX: upgrading from read fault to write fault
	outp = (u8 *)parm + 0x0200;
	test(code, 1, 0x1000, parm, true, outp, false);
	free(parm);
	...
	test(code, 2, 0x1000, parm, false, outp, false);
	free(outp);
	free(parm);
#endif
	...
	test(code, 2, 0x2000, parm, false, outp, false);
	free(outp);
	free(parm);
#endif
	...
	parm = aligned_alloc(4096, 4096);
	outp = aligned_alloc(4096, 4096);
	test(kernel[LDC_LDG_STG], 2, 0x0000, parm, false, outp, false);
	free(outp);
	free(parm);
#endif

	//XXX: gpu write to read-only page?
#if 1 /* volta channel recovery not terribly reliable yet... */
	parm = aligned_alloc(4096, 4096);
	outp = aligned_alloc(4096, 4096);
	mprotect(outp, 0x1000, PROT_READ);
	test(code, 1, 0x1000, parm, false, outp, true);
